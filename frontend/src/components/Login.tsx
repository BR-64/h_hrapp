...
export const Login: React.FC<LoginProps> = ({ onLogin, onRegister }) => {
  ...
  useEffect(() => {
    Promise.all([settingsApi.getDepartments(), settingsApi.getPositions()])
      .then(([depts, pos]) => {
        setDepartments(depts);
        setPositions(pos);
        if (depts.length > 0) setDept(depts[0]);
        if (pos.length > 0) setPos(pos[0]);
      })
      .catch(() => {});
  }, []);
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await authApi.register({ name, email, password, department: dept, position: pos });
